...
class BCGFontFile implements BCGFont {
    ...
    public function draw($im, $color, $x, $y) {
        ...
        // Modified by MOn
		imagettftext($im, $this->size, $this->rotationAngle, $drawingPosition[0], $drawingPosition[1]-2, $color, $this->path, $this->text);
		//imagettftext($im, 16, $this->rotationAngle, 0, $drawingPosition[1]-3, $color, $this->path, $this->text);
		// added by Mon
		imagettftext($im, 14, $this->rotationAngle, 5, 20, $color, $this->path, $this->pateint_lname.", ".$this->pateint_fname);
		imagettftext($im, 14, $this->rotationAngle, 5, 40, $color, $this->path, $this->pateint_mname);
		
		$white = imagecolorallocate($im, 255, 255, 255);
		$black = imagecolorallocate($im, 0, 0, 0);
		// Draw a black rectangle
		imagefilledrectangle($im, 337, 0, 322, 60, $black);
		imagettftext($im, 8, 270, 326, 5, $white, $this->path, $this->specimen);
		imagettftext($im, 8, 270, 310, 4, $black, $this->path, $this->datetime);
    }
